"""
oar_session_agent._oar_asia_teyit — backtest-kanıtlı OAR-CORE trio'sunun canlı
...
from oar_session_agent import _oar_asia_teyit
...
def test_yetersiz_veri_none():
    assert _oar_asia_teyit([], 110, 100) is None
    assert _oar_asia_teyit(_taban(5), 110, 100) is None
    assert _oar_asia_teyit(_taban(), 0, 0) is None
...
def test_ust_supurme_yuksek_hacim_short():
    ...
    sonuc = _oar_asia_teyit(mumlar, 110, 100)
...
def test_supurme_var_ama_hacim_dusuk_none():
    ...
    assert _oar_asia_teyit(mumlar, 110, 100) is None
