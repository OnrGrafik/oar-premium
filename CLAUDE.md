...
- ⚠️ CANLI VERİ KISITI (KALICI — dürüst sınır): canlı ortamda tick/aggTrades/footprint/CVD verisi YOK (yalnız `_ohlcv_al` klines). Bu yüzden canlı FADE girişi `_oar_asia_teyit` klines-PROXY'sidir; gerçek şampiyon footprint bloklarının (absorpsiyon/kalıcılık) canlı hesabı FİZİKSEL OLARAK İMKÂNSIZ. "Canlıyı birebir şampiyon yap" kovalamayın — canlı tick veri kaynağı (Binance WS aggTrades vb.) eklenmeden yapılamaz. Backtest "en iyi getiri" = per-sembol KAPI + kanıtlı şampiyon; canlı bunu klines-proxy ile yaklaşık uygular. Kanıt (serap DSR 1.0) backtest bloklarına aittir; canlı proxy ayrı, forward-test ile izlenir.
