...
async def lider_karar_uret(sembol: str = "BTCUSDT", api_key: str = "") -> dict:
    ...
    # CIO paper trade KALDIRILDI (ANAYASA #9: OAR-dışı enerji). OAR forward-test
    # oar_paper_box + oar_altcoin_sistem ile yapılıyor. Karar yalnız arşivlenir.
    try:
        import persistence as _db
        _db.karar_kaydet(karar_verisi)
    except Exception:
        pass
    sonuc["paper_trade"] = {"acildi": False, "neden": "CIO paper kaldırıldı (OAR-only)"}
    ...
    while True:
        try:
            ...
            ai = await _hizli_ai(
                ...
                f"{oar_kural_ctx}\n"
                f"OAR strateji özeti: Asia Range (00:00-04:00 UTC) fib; PER-SEMBOL kapı — her coin "
                f"KENDİ Asia ≥%1 ise o coinde fade günü (BTC≥%1→BTC'de, ETH≥%1→ETH'de), altı → o coinde trade yok; "
                f"kanıtlı şampiyon = poc_taraf+footprint_absorpsiyon+footprint_kalicilik "
                f"(Asia ekstremlerinde fade; PF ~3.1, +273%). "
                f"Trend-devam günü (o coinin Asia <%1) fade alınmaz.\n"
                f"{oar_kanit_ctx}\n"
